...
Caret package has functions to define the tune grid for each model in order to obtain hyper parameter optimization. The code below will enable the tuning of the parameter for the Support Vector Machines (SVM)
```{r}
tuneGrid_svm <- expand.grid(C= c(0.1, 1, 10, 100),
                            sigma = c(0.01, 0.1, 1))
```
### Alternative Tune for the SVM
```{r}
#tuneGrid_svm <- expand.grid(C = seq(1, 100, by = 2),
                            #sigma = seq(0.01, 1, by = 0.05))
...
### Plot the Model
```{r, fig.height=5}
plot(SvmModel, main = "A plot of Accuracy against the Regularization Parameter for the SVM")
```
...
Similar to SVM, caret package has a function to hyper parameter tuning in decision tree model to optimize the parformance of the model. The code will create the parameter tuning scheme so as the model developed has the optimized performance.
...
### Alternative Decision Tree Plot
```{r, fig.height=6}
...
```{r}
tuneGrid_rf <- expand.grid(mtry = seq(1, 20, by = 1))
```
...
### Hyper Parameter Tuning
Logistic regression typically doesn't require extensive hyper parameter tuning unless one is using regularization techniques like; ridge or lasso. For a regular logistic regression, one should focus on data preprocessing and feature selection rather than hyper parameter tuning. 
For Ridge logistic regression, the following scheme will help in hyper parameter tuning

```{r}
tuneGrid_rlr <- expand.grid(lambda = seq(0,1, length = 10))
```
...
### Hyper parameter Tuning
Like all other models, caret has a way of tuning hyper parameter in the knn model. The parameter of interest in knn is the values of k, which has to be optimum. The code below will tune the parameter of interest
...
### Hyper Parameter Tuning
```{r}
tuneGrid_ann <- expand.grid(size = c(3,5,7), decay = c(0.1, 0.5, 0.9))
```

#### expand.grid()
Function Purpose: The expand.grid() function in R is used to create a data frame from all combinations of the supplied vectors. It’s useful for generating a grid of hyperparameters for model tuning.
2. size = c(3, 5, 7)

#### size: 
This represents the number of neurons (or units) in the hidden layer of the neural network.

#### Values Provided:
3: The hidden layer has 3 neurons.
5: The hidden layer has 5 neurons.
7: The hidden layer has 7 neurons.
3. decay = c(0.1, 0.5, 0.9)

##### decay: 
This parameter controls the weight decay, which is a form of regularization used to prevent overfitting by penalizing large weights in the network.

##### Values Provided:
0.1: Low regularization.
0.5: Medium regularization.
0.9: High regularization.

### How expand.grid() Works with These Parameters
Grid Creation: The expand.grid() function creates a data frame where each row represents a unique combination of the hyperparameters size and decay. The resulting grid will have all possible combinations of these values, which can then be used during the training process of the neural network model.
...
### Hyper Parameter Tuning
```{r}
tuneGrid_nb <- expand.grid(fL = c(0, 0.5, 1.0),
                           usekernel = c(TRUE, FALSE),
                           adjust = c(0, 0.5, 1.0))
```

#### 1. expand.grid()
Function Purpose: The expand.grid() function in R creates a data frame from all combinations of the supplied vectors or factors. It’s particularly useful for generating a grid of hyperparameters that can be used in model tuning.
2. fL = c(0, 0.5, 1.0)

#### fL: 
This represents a hyperparameter used in Naive Bayes modeling, particularly in functions from packages like e1071 or caret.
Purpose of fL: It stands for "Laplace smoothing" or "fL" (feature level), which is a technique used to handle zero frequencies in categorical data. When a category in the data does not appear in the training set, Laplace smoothing avoids zero probabilities by adding a small value (like 1 or 0.5) to each category count.

#### Values Provided:
0: No smoothing.
0.5: Moderate smoothing.
1.0: Full smoothing.
3. usekernel = c(TRUE, FALSE)

##### usekernel: 
This parameter determines whether a kernel density estimator is used instead of a Gaussian distribution for continuous features.
Purpose of usekernel:

#### TRUE: 
Use a kernel density estimator, which is a non-parametric way to estimate the probability density function of a random variable.

#### FALSE: 
Use the default Gaussian distribution assumption for continuous features.
4. adjust = c(0, 0.5, 1.0)

#### adjust:
This parameter is associated with the kernel density estimator. It adjusts the bandwidth of the kernel density estimate, which influences how smooth the estimated density function is.

#### Purpose of adjust:
0: No adjustment to the bandwidth.
0.5: Reduce the bandwidth by half, resulting in a sharper density estimate.
1.0: Use the default bandwidth, resulting in the standard smoothing.

### How the Hyper parameter is done
```{r}
tuneGrid_nb
```

In the context of Naive Bayes, the term "kernel" typically refers to the use of kernel density estimation (KDE) when estimating the probability distribution of continuous features. The "kernel" in Naive Bayes is not about using multiple kernels but rather whether to use a kernel-based method or not for estimating the distribution of continuous variables.

### Key Points:
#### No Kernel (usekernel = FALSE): 
When usekernel = FALSE, Naive Bayes assumes that the continuous features follow a Gaussian (normal) distribution.

#### Kernel (usekernel = TRUE): 
When usekernel = TRUE, the algorithm uses kernel density estimation to estimate the probability distribution of the continuous features, which does not assume any specific parametric form (like the Gaussian distribution).

## How Many Kernels?
Single Kernel Per Feature: 
If usekernel = TRUE, a single kernel density estimation method is applied to each continuous feature individually to estimate its distribution. This means each continuous feature will have its distribution estimated using KDE, but the number of kernels corresponds to the number of continuous features in the dataset, not a specific number of different kernel types.

Multiple Features: 
If your dataset has multiple continuous features, KDE will be applied independently to each feature.

### Summary:
The term "kernel" in Naive Bayes refers to the method of estimating the probability distribution of continuous features. If usekernel = TRUE, the Naive Bayes classifier will use kernel density estimation for each continuous feature in the dataset, but it's not about using multiple kernel functions; it's about using a kernel density estimation for each feature.

### Develope the Naive Bayes Model
```{r}
set.seed(123)
NBModel <- train(factor(Malaria.Test) ~ ., 
                 data = train, method = "nb",
                 trControl = control,
...
### Extract the Model's Performance Metrics
```{r}
...
# Alternatively using ggplot function
varImp <-varImp(NBModel)
ggplot(varImp, aes(x = reorder(Variable, Importance), y = importance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  xlab("Variable") +
  ylab("Importance") +
  ggtitle("Feature Importance Plot for the NB Model")
```

### ROC and AUC for the Naive Bayes Model
```{r, fig.height=5}
# Make predictions on the test set using type='prob'
...
# Plot the ROC curve
plot(perf_nb, colorize = TRUE, main = "ROC Curve for the Naive Bayes Model Model")
# Compute AUC
...
#### Hyper Parameter Tuning
Linear Discriminant Analysis (LDA) has few hyper parameter, so tuning parameter is not as common as in other algorithms. The main focus is usually on data pre-processing. No typical parameter tuning is needed for LDA. LDA is parameter-free in terms of model complexity

### Model Estimation
```{r}
set.seed(123)
LDAModel <- train(factor(Malaria.Test)~., 
                  data=train, 
                  method="lda", 
                  trControl=control)
LDAModel
...
# Alternatively using ggplot function
varImp <-varImp(LDAModel)
ggplot(varImp, aes(x = reorder(Variable, Importance), y = importance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  xlab("Variable") +
  ylab("Importance") +
  ggtitle("Feature Importance Plot for the LDA Model")
```

### ROC and AUC for the LDA Model
```{r, fig.height=5}
# Make predictions on the test set using type='prob'
...
# Plot the ROC curve
plot(perf_lda, colorize = TRUE, main = "ROC Curve for the LDA Model")
# Compute AUC
...
Function Purpose: The expand.grid() function in R creates a data frame from all combinations of the input vectors. Each row in the resulting data frame represents a unique combination of the parameters listed.
In Context: It generates a grid of hyperparameter values to be used for tuning the LVQ model.

### size = c(5, 10, 15):
Purpose: The size parameter refers to the number of prototypes (or reference vectors) in each class. These prototypes represent the centroids or typical examples for each class.
Values: The code tries three different values for size: 5, 10, and 15. This means that the model will be trained and evaluated with 5, 10, and 15 prototypes per class.

### k = c(1, 3, 5):
Purpose: The k parameter typically represents the number of nearest neighbors to consider in the LVQ algorithm. It determines how many neighbors are used to classify a new instance during the training process.
Values: The code tries three different values for k: 1, 3, and 5. This means that the model will be trained and evaluated with 1, 3, and 5 neighbors.

### learning_rate = c(0.01, 0.05, 0.1):
Purpose: The learning_rate parameter controls how quickly the model adjusts the prototypes during training. A smaller learning rate results in slower, more gradual updates, while a larger learning rate results in quicker, more significant updates.
Values: The code tries three different values for learning_rate: 0.01, 0.05, and 0.1. This means that the model will be trained and evaluated with these different learning rates.

### What This Code Does:
Grid of Hyperparameters: The expand.grid() function creates a grid of all possible combinations of the size, k, and learning_rate parameters. This grid will be used to train and evaluate the LVQ model for each combination, allowing you to identify the best set of hyperparameters based on model performance.

Hyperparameter Tuning: By trying different combinations of these parameters, the goal is to find the optimal set of parameters that results in the best performance of the LVQ model on the given data.
...
# Alternatively using ggplot function
varImp <-varImp(LVQModel)
ggplot(varImp, aes(x = reorder(Variable, Importance), y = importance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  xlab("Variable") +
  ylab("Importance") +
  ggtitle("Feature Importance Plot for the LVQ Model")
```
...
### Hyper Parameter Tuning
```{r}
tuneGrid_bag <- expand.grid(cp = seq(0, 0.1, length = 10 ))



tuneGrid_bagging <- expand.grid(
  mtry = seq(1, 20, by = 1),            # Number of variables randomly sampled as candidates at each split
  nbagg = c(10, 20, 50, 100, 500),              # Number of bootstrap samples or trees
  minsplit = seq(10, 50, by = 10),      # Minimum number of observations to split a node
  minbucket = seq(5, 25, by = 5)        # Minimum number of observations in any terminal node (leaf)
)
tuneGrid_bagging
```
...
```{r}
tuneGrid_boo <- expand.grid(iter = c(50, 100, 150),
                            maxdepth = seq(1, 21, by=2),
                            nu = seq(0.01, 0.1, by = 0.01))
tuneGrid_boo
```

expand.grid(...):

This function is used to create a data frame from all combinations of the supplied vectors or factors. Essentially, it generates all possible combinations of the hyperparameter values you specify, creating a "grid" of parameters that will be tested during model training.
iter = c(50, 100, 150):

iter likely refers to the number of boosting iterations (or rounds). Boosting algorithms add trees iteratively to improve model performance, so iter controls how many trees will be added.
The values 50, 100, and 150 indicate that you want to test models with 50, 100, and 150 boosting iterations, respectively.
maxdepth = seq(1, 21, by=2):

maxdepth specifies the maximum depth of the trees in the boosting algorithm. The depth of a tree determines how complex the model can be, with deeper trees potentially capturing more details but also increasing the risk of overfitting.
seq(1, 21, by=2) generates a sequence of numbers starting at 1 and ending at 21, incrementing by 2. This means the grid will include trees with depths of 1, 3, 5, ..., up to 21.
nu = seq(0.01, 0.1, 1):

nu likely refers to the learning rate (or shrinkage parameter) in the boosting algorithm. The learning rate controls how much each tree influences the final model. Smaller values of nu lead to slower learning and more conservative models, but potentially more accurate ones when more iterations are used.
seq(0.01, 0.1, 1) is intended to generate a sequence of learning rates starting at 0.01 and ending at 0.1. However, the syntax here is incorrect for generating a sequence. The correct syntax should be seq(0.01, 0.1, by=0.01), which generates values 0.01, 0.02, ..., up to 0.1.

### View the Grid Search
```{r}
tuneGrid_boo
```
